...
s2p_params = {'cell_soma_size':20, # microns
              'max_reg_shift':20, # microns
              'block_size':np.asarray([60,60]), #microns
              'smooth_sigma':1, # microns
              'smooth_sigma_time':.1, #seconds,
              'basedir':'/home/rozmar/Data/Calcium_imaging/raw/Genie_2P_rig/rozsam', # folder where the raw ephys and movies are found
              'basedir_out':'/home/rozmar/Data/Calcium_imaging/suite2p_fast/Genie_2P_rig/rozsam',
              'basedir_out_slow':'/home/rozmar/Data/Calcium_imaging/suite2p/Genie_2P_rig/rozsam'} # folder where the suite2p output is saved

metadata_params = {'notebook_name':"Genie Calcium Imaging",
                   'metadata_dir':"/home/rozmar/Data/Calcium_imaging/Metadata"}

ephys_params = {'basedir':'/home/rozmar/Data/Calcium_imaging/raw/Genie_2P_rig/rozsam'}
...
F_filter_sigma = .01 #sigma for gaussian filtering in seconds
...
time_forward = .2 # seconds from event start when cutting out ephys and ophys responses

min_baseline_time =1 #seconds before an event - for bucketing
integration_time = .05 # seconds after the last AP - for bucketing
    ...
    for df_run in df_session.iterrows():
        ...
        if imaging_quality ==1:
            ...
            for ephysfile in ephysfiles_real:
                #% get ephys data
                voltage, frame_trigger, sRate, recording_mode =  utils_ephys.load_wavesurfer_file(os.path.join(celldir,ephysfile))
    ...
    if len(idxs)>0:
        ...
        if plot_stuff:
            ...
            for framerate in unique_framerates:
                ...
                x = np.asarray(dFF_t_all)[idxs][framerate_idx].T
                y = np.asarray(dFF_all)[idxs][framerate_idx].T
                ax_ophys.plot(x,y)
                ax_ophys.plot(np.nanmean(x,1),np.nanmean(y,1),'k-',lw=2)
                
                y = np.asarray(dFF_local_all)[idxs][framerate_idx].T
...
ax_amplitudes.plot(np.asarray(event_ap_num)[idxs],np.asarray(event_amplitude_local)[idxs],'ko',ms=3)
ax_amplitudes.plot(apnums,amplitude_mean,'ro-',lw=3)
ax_amplitudes.errorbar(apnums,amplitude_mean,amplitude_std,color='red',lw=3)
ax_amplitudes.set_ylim([-.1, .5])

ax_amplitudes_new = fig.add_subplot(132)
ax_amplitudes_new.plot(np.asarray(event_ap_num)[idxs],new_amplitudes[idxs],'bx',ms=3)
ax_amplitudes_new.errorbar(apnums,amplitude_mean_new,amplitude_std_new,color='blue',lw=3)
ax_amplitudes_new.set_ylim([-.1, .5])
...
for cellid in cellids:
    print(cellid)
    idxs = np.where((np.asarray(event_r_neu_cell_id) == cellid) & (np.asarray(event_r_neu_corrcoef)>.4))[0] 
    if len(idxs)>0:
       y,x = np.histogram(np.asarray(event_r_neu)[idxs],bins = 10, range=[0,2])
       if len(Y)>0:
           plt.bar(np.mean([x[:-1],x[1:]],0),y,bottom=sum(Y), width = .2)
       else:
